...
#' @param dt_dist dwell time distribution, which should be either "log" or
#' "shiftpoisson".
...
hsmmsim <- function(n, M, prior, dt_dist, dt_parm, 
  ...
  if(length(prior)!=M | length(emit_parm)!=M | length(zeroprop)!=M |
     nrow(tpm_parm)!= M | ncol(tpm_parm)!=M | length(dt_parm) != M) stop("The dimension of the initial value does not equal M!") 
